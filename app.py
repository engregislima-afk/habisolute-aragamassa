# app.py — Rupturas de Argamassa (Habisolute style)
# -------------------------------------------------
...
# PDF (ReportLab) para "imprimir" a lista
...
st.markdown(f"<h1>Rupturas de Argamassa</h1>", unsafe_allow_html=True)
...
def _dp(lst):
    ...
    if len(lst) == 1:
        return 0.0
    # dp populacional para relatório simples
    return pstdev(lst)

def build_pdf(obra, data_obra, df: pd.DataFrame) -> bytes:
    """
    ...
    title = Paragraph(f"<para align='center'><b>Rupturas de Argamassa — Lote</b></para>", styles["Title"])
    info  = Paragraph(
    ...
    data_table = [
        ["#", "Código CP", "Modo entrada", "Entrada", "Área (cm²)", "kgf/cm²", "kN/cm²", "MPa"]
    ]
...
if st.session_state.get("pdf_bytes", None):
...
# Rodapé simples
st.caption("Conversões: 1 kgf/cm² = 0,00980665 kN/cm² = 0,0980665 MPa. • © Habisolute")
