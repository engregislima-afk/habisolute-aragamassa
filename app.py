...
# Tema preto/laranja (#d75413) e claro/laranja, gráfico de pontos (Altair),
# PDF com gráfico quando houver fpdf2+plotly+kaleido,
# e fallback automático para HTML com gráfico SVG quando não houver backend.
...
# ---------------- PDF backend detection (reportlab → fpdf2 → none) ----------------
...
def _try_import_pdfs():
    ...
    try:
        ...
        from reportlab.platypus import SimpleDocTemplate, Table, TableStyle, Paragraph, Spacer, Image  # noqa
...
if "html_fallback_bytes" not in st.session_state: st.session_state.html_fallback_bytes = None
...
with st.sidebar:
    ...
    st.session_state.footer_text = st.text_area(
        "Observações (norma, técnico resp., etc.)",
        st.session_state.footer_text, height=90
    )
...
st.caption("Entrada: **carga de ruptura (kgf)**. Área por obra. Saídas: **kN/cm²** e **MPa**. Gráfico de pontos. PDF/HTML com gráfico.")
...
# ---------------- PNG do gráfico (Plotly + Kaleido; usado no PDF) ----------------
def chart_png_from_df(df: pd.DataFrame) -> bytes | None:
    ...
    ymax = max(mpa)*1.15 if mpa else 1
...
# ---------------- Fallback: gráfico SVG (sem dependências) ----------------
def svg_scatter_from_df(df, width=900, height=360, margin=50):
    if df.empty: return "<svg width='1' height='1'></svg>"
...
def build_pdf(obra: str, data_obra: date, area_cm2: float, df: pd.DataFrame,
    ...
    if PDF_BACKEND == "fpdf2":
        ...
        pdf.ln(3)
        headers = ["#", "Código CP", "Carga (kgf)", "Área (cm²)", "kN/cm²", "MPa"]; widths=[8,52,28,22,28,24]
...
with st.form("cp_form", clear_on_submit=True):
    ...
    add = st.form_submit_button("Adicionar CP ao lote", disabled=(qtd>=12 or not st.session_state.obra))
    if add:
        if not st.session_state.obra: st.error("Defina os dados da obra antes."); 
        elif not codigo_cp.strip():  st.error("Informe o código do CP.")
        elif carga_kgf <= 0:        st.error("Informe uma carga > 0.")
        else:
...
if st.session_state.registros:
    ...
    cA,cB = st.columns(2)
    with cA: st.metric("Média (kN/cm²)", f"{mean(df['kn_cm2']):.4f}")
    with cB: st.metric("Média (MPa)",    f"{mean(df['mpa']):.3f}")
    ...
    points = (alt.Chart(chart_df).mark_point(size=90, filled=True, color=ACCENT)
...
h2{{margin:0 0 8px}} section{{margin:16px 0}}
</style></head><body>
